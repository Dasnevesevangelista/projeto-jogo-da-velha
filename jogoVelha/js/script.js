...
function hadleWin(regions){
  regions.forEach(function (region) {
    document.querySelector('[data-region="'+ region +'"]').classList.add('win')
  })
  const playerName = document.getElementById(turnPlayer).value
  alert('Vitória de ' + playerName + '\n Clique em começa para uma nova partida!' )
}


function handleBoardClick(ev) {
  // Obtém os índices da região clicada
  const nomeJogador1 = document.getElementById('player1').value
  const nomeJogador2 = document.getElementById('player2').value

  if(nomeJogador1 == '' && nomeJogador2 == ''){
    alert('Você precisa digitar o nome dos jogadores para começar a partida')
  } else {
